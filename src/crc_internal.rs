...
mod tests {
    ...
    #[test]
    /// Verifies CRC validation and extraction succeeds on valid data.
    fn test_validate_and_extract_ok() {
        let data = [0x12_u8, 0x34_u8];
        let crc = generate_crc(&data);
        let input = [data[0], data[1], crc];

        let mut output = [0u16; 1];
        let len = validate_and_extract_data(&input, &mut output).unwrap();

        assert_eq!(len, 1);
        assert_eq!(output[0], 0x1234);
    }

    #[test]
    /// Verifies CRC validation fails on invalid CRC.
    fn test_invalid_crc() {
        let input = [0x12_u8, 0x34_u8, 0x00_u8];
        let mut output = [0u16; 1];

        let err = validate_and_extract_data(&input, &mut output).unwrap_err();
        assert_eq!(err, CrcError::InvalidCrc);
    }

    #[test]
    /// Verifies CRC validation fails on invalid input length.
    fn test_invalid_input_length() {
        let input = [0x12_u8, 0x34_u8];
        let mut output = [0u16; 1];

        let err = validate_and_extract_data(&input, &mut output).unwrap_err();
        assert_eq!(err, CrcError::InvalidInputLength);
    }

    #[test]
    /// Verifies CRC validation fails when output buffer is too small.
    fn test_buffer_too_small() {
        let data = [0x12_u8, 0x34_u8];
        let crc = generate_crc(&data);
        let input = [data[0], data[1], crc, 0x56_u8, 0x78_u8, crc];
        let mut output = [0u16; 1];

        let err = validate_and_extract_data(&input, &mut output).unwrap_err();
        assert_eq!(err, CrcError::BufferTooSmall);
    }
}
